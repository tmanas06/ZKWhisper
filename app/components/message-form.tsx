...
const prompts = (companyName: string, isWallet: boolean = false) => {
  if (isWallet) {
    return [
      `What's on your mind?`,
      `Share your thoughts anonymously...`,
      `What would you say if you weren't being watched?`,
      `Speak freely, your identity is protected.`,
...
const MessageForm: React.FC<MessageFormProps> = ({ isInternal, onSubmit }) => {
  ...
  const welcomeMessage = `
    Sign in with your Google work account or connect your Web3 wallet to anonymously post messages.
  `;
          ...
          {isRegistered && (
            <div className="message-form-footer-buttons">
              <button
                ...
                onClick={() => handleSignIn(currentProvider || "google-oauth")}
                tabIndex={-1}
              >
                {isRegistering ? (
